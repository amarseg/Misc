...
#v3 -> type of symbol changes (density of the dots is easier to see)
...
#####computes spread for each of the 'toDo' datasets
 ...
 for (j in 1:toDo)
 {
  ...
  ma=max(data[,2],na.rm=T)
  mi=min(data[,2],na.rm=T)
...
###
  ...
  for (i in inter)
  {
   ...
   if(nu > 1)
   {
    ###edited 270513###dat=((seq(1,nu,1)-median(seq(1,nu,1,na.rm=TRUE)))/spread[j]) + (pwidth * (j-1))
    dat=((seq(1,nu,1)-median(seq(1,nu,1)))/spread[j]) + (pwidth * (j-1))
...
#######plots data
  ...
  if((j==1) & (add==FALSE))
  {
   plot(data[,1],data[,2], ylim=ylim, xlim=xlim, col=col[j],pch=1, xaxt="n",xlab=xlab, ylab=ylab, main=main,cex=cex,cex.lab=cex.lab,cex.axis=cex.axis)
  }
 
  else if ((j!=1)|(add==TRUE))
  {
   lines(data[,1],data[,2], col=col[j],pch=1, type="p",cex=cex)
  }
