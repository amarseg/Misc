...
medias <- colMeans(new_order)
remedia <- c(1:12)
for(i in 1:12)
{
	remedia[i] <- mean(c(medias[i],medias[i+12],medias[i+24]) )
}

plot(y = remedia, x = c(0:11), type = 'l', lwd = 3, ylim = c(-2,2), col = 'red')
time = rep(0:11,3)
points(y = medias, x = time)
...
medias_rna <- colMeans(norm_sam)

remedia_rna <- c(1:12)
for(i in 1:12)
{
	remedia_rna[i] <- mean(c(medias_rna[i],medias_rna[i+12],medias_rna[i+24]) )
}

plot(y = remedia_rna, x = c(0:11), type = 'l', lwd = 3, ylim = c(-2,2), col = 'red')
time = rep(0:11,3)
points(y = medias_rna, x = time)
